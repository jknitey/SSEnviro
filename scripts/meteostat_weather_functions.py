...
def get_weather_data(id, type, latitude, longitude, start_date, end_date) -> pd.DataFrame:
    ...
    if type == 'hourly':
        ...
        data_col = ['temp', 'dwpt', 'rhum', 'prcp', 'snow', 'wdir', 'wspd', 'wpgt', 'pres', 'tsun', 'coco']
    elif type == 'daily':
        # Get daily data
        final_weather_data = Daily(place, start, end)
        final_weather_data = final_weather_data.fetch()
        data_col = ['tavg', 'tmin', 'tmax', 'prcp', 'snow', 'wdir', 'wspd', 'wpgt', 'pres', 'tsun']
    elif type == 'monthly':
        # Get daily data
        final_weather_data = Monthly(place, start, end)
        final_weather_data = final_weather_data.fetch()
        data_col = ['tavg', 'tmin', 'tmax', 'prcp', 'wspd', 'pres', 'tsun']
    else:
